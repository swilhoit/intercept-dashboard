...
# Heatilator configuration
HEATILATOR_CONFIG = {
    'base_url': 'https://heatilatorfireplacedoors.com',
    'consumer_key': os.environ.get('HEATILATOR_CONSUMER_KEY', 'ck_440a83e0aa324f7a0dcb10b07710239b1af741d0'),
    'consumer_secret': os.environ.get('HEATILATOR_CONSUMER_SECRET', 'cs_893f884fb20e5bc9c2655188c18c08debebf7bb7')
}

@functions_framework.http
def heatilator_daily_sync(request):
    """Cloud function to sync Heatilator daily orders"""
    try:
        print("🔥 Starting Heatilator daily sync...")
        ...
        if orders:
            ...
            if success:
                return {
                    'status': 'success',
                    'message': f'Processed {len(orders)} Heatilator orders',
            ...
            return {
                'status': 'success', 
                'message': 'No new Heatilator orders found',
                'date_range': f'{start_date} to {end_date}'
            }, 200
            
    except Exception as e:
        print(f"❌ Error in heatilator_daily_sync: {e}")
...
def fetch_recent_orders(start_date, end_date, max_pages=5):
    ...
    print(f"📥 Fetching Heatilator orders from {start_date} to {end_date}")
    ...
    for page in range(1, max_pages + 1):
        ...
        cmd = [
            ...
            f'{HEATILATOR_CONFIG["base_url"]}/wp-json/wc/v3/orders?per_page=50&page={page}',
            '-u', f'{HEATILATOR_CONFIG["consumer_key"]}:{HEATILATOR_CONFIG["consumer_secret"]}'
        ]
...
def process_and_insert_orders(orders, start_date):
    ...
    try:
        ...
        delete_query = f"""
        DELETE FROM `{os.environ.get('GOOGLE_CLOUD_PROJECT')}.woocommerce.heatilator_daily_product_sales`
        WHERE order_date >= '{start_date}'
        """
        print(f"🗑️ Clearing Heatilator data from {start_date}...")
        ...
        if daily_products:
            ...
            if rows_to_insert:
                ...
                if not errors:
                    print(f"✅ Inserted {len(rows_to_insert)} Heatilator product records")
...
def update_master_table(client, daily_products):
    ...
    for order_date, total_sales in total_sales_by_date.items():
        try:
            ...
            WHEN MATCHED THEN
              UPDATE SET 
                woocommerce_sales = COALESCE(T.woocommerce_sales, 0) - COALESCE(T.heatilator_sales, 0) + S.heatilator_sales,
                total_sales = COALESCE(T.amazon_sales, 0) + 
                             (COALESCE(T.woocommerce_sales, 0) - COALESCE(T.heatilator_sales, 0) + S.heatilator_sales),
                heatilator_sales = S.heatilator_sales
            WHEN NOT MATCHED THEN
              INSERT (date, woocommerce_sales, amazon_sales, total_sales, heatilator_sales)
              VALUES (S.date, S.heatilator_sales, 0, S.heatilator_sales, S.heatilator_sales)
            """
