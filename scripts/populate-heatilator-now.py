...
def fetch_heatilator_orders(per_page=50, max_pages=50):
    """Use curl to fetch Heatilator orders"""
    all_orders = []
    
    credentials = "ck_440a83e0aa324f7a0dcb10b07710239b1af741d0:cs_893f884fb20e5bc9c2655188c18c08debebf7bb7"
    
    for page in range(1, max_pages + 1):
        print(f"📄 Fetching Heatilator page {page}...")
        
        cmd = [
            'curl', '-s',
            f'https://heatilatorfireplacedoors.com/wp-json/wc/v3/orders?per_page={per_page}&page={page}',
            '-u', credentials
        ]
        ...
        try:
            ...
            if len(filtered_orders) == 0:
                if not hasattr(fetch_heatilator_orders, 'empty_count'):
                    fetch_heatilator_orders.empty_count = 0
                fetch_heatilator_orders.empty_count += 1
                if fetch_heatilator_orders.empty_count >= 3:
    ...
    print(f"✅ Total completed Heatilator orders from 2024+: {len(all_orders)}")
    return all_orders
...
def process_and_insert_orders(orders):
    ...
    print(f"📊 Processing {len(orders)} Heatilator orders...")
    ...
    print(f"📤 Inserting {len(rows_to_insert)} Heatilator daily product records...")
    ...
    try:
        ...
        """
        print("🗑️ Clearing existing Heatilator data...")
        ...
        if not errors:
            print(f"✅ Successfully inserted {len(rows_to_insert)} Heatilator records")
...
def main():
    print("🔥 Starting Heatilator data population...")
